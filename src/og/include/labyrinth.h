...
class Labyrinth {
    private:
        // Atributos
        std::vector<CellVector> labyrinth_; // Matriz que representa el laberinto
        int rows_, columns_;                // Número de filas y columnas del laberinto
        Cell start_node_, end_node_;        // Nodos inicial y final
        int chosen_heuristic_{1};           // Heurística elegida (1: Manhattan, 2: Euclídea)
    ...
    public:
        ...
        std::string PrintLabyrinth(CellVector open_nodes = {}, CellVector closed_nodes = {}, Cell current_node = Cell(), CellVector path = {}) const;
