 ...
 * @return false 
 */
std::string Labyrinth::PrintLabyrinth(CellVector open_nodes, CellVector closed_nodes, Cell current_node, CellVector path) const {
...
void Labyrinth::ChangeNode(bool final) {
    ...
    std::cout << "The " << (final ? "final" : "initial") << " node has been set to" << " (" << i_pos << ", " << j_pos << ")\n"; // Se imprime el nodo modificado
}
...
void Labyrinth::CalculateValues(Cell& node, Cell& current_node) const {
    if (current_node.IsDiagonal(node, labyrinth_)) {               // Actualiza el valor g(n) del nodo:
        ...
        node.SetGValue(current_node.GetGValue() + 5);            // Si el nodo es adyacente, el coste es 5
    }
...
Instance Labyrinth::ModAStarSearch() const {
