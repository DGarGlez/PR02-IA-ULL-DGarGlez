 ...
 * @param kind 
 */
Cell::Cell(int i_pos, int j_pos, int kind): i_pos_(i_pos), j_pos_(j_pos), kind_(kind) {}
...
std::string Cell::GetPosString() const {
    return "(" + std::to_string(i_pos_) + ", " + std::to_string(j_pos_) + ")";   
}
...
void Cell::CalculateHeuristic(Cell end_node, int chosen_heuristic) {
    switch (chosen_heuristic) {
        ...
        case 1:  // Distancia Manhattan
            h_value_ = (std::abs(i_pos_ - end_node.GetIPos()) + std::abs(j_pos_ - end_node.GetJPos())) * 3;
            break;
...
std::string Cell::PrintValues() const {
    return "f: " + std::to_string(f_value_) + ", g: " + std::to_string(g_value_) + ", h: " + std::to_string(h_value_);
}
